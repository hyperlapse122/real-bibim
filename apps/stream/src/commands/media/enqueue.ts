...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName('url').setDescription('URL of the music').setRequired(true),
  );
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    await interaction.editReply(
      `[${info.title}](${info.original_url}) by ${(info.uploader_url && `[${info.uploader}](${info.uploader_url})`) || info.uploader} is enqueued!`,
    );
  } catch (e) {
    console.error(e);
