...
const voiceConnectionAtomFamily = atomFamily((args: VoiceConnectionArgument) => {
    ...
    const effect = atomEffect((get, set) => {
        get(internalAtom).then(c => c.addListener(VoiceConnectionStatus.Destroyed, () => {
            set(internalAtom)
        }))
    })
...
}, (a, b) => a.guildId === b.guildId && a.channelId === b.channelId && (a.adaptorCreator === b.adaptorCreator || !b.adaptorCreator));
